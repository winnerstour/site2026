...
function buildHotelBookingUrl(hotel, eventMeta) {
  if (!hotel) return '#';
  const baseUrl = 'https://www.comprarviagem.com.br/busca-hotel';
...
document.addEventListener('DOMContentLoaded', async function () {
  ...
  try {
    ...
    if (!response.ok) {
      console.warn('Falha ao tentar', jsonPath, 'status', response.status);

      const fallbackPath = fixPath('/eventos/' + slug + '.json');
    ...
    (async function () {
      ...
      hotelsSection.innerHTML = `
        ...
        <p class="wrap section-lead" style="margin-bottom: 24px;">
          Nossos roteiros e hotéis recomendados foram desenhados para quem viaja a trabalho, otimizando seu tempo e custo durante a feira. Veja mais detalhes abaixo e escolha o que mais combina com a sua agenda. Se preferir,
          <a id="hotelsWhatsLink" href="#" target="_blank" rel="noopener" style="color: var(--brand); font-weight: 700;">
            clique aqui para simular voos + outros hotéis disponíveis na nossa loja virtual
          </a>.
        </p>
        <div id="hotelsWrapper" class="hotels-wrapper">
          <div id="hotelsCarouselContainer" class="cl-track"></div>
          <button class="carousel-nav prev" id="prevHotels">
            <svg viewBox="0 0 24 24"><path fill="currentColor" d="M15.41,16.58L10.83,12L15.41,7.41L14,6L8,12L14,18L15.41,16.58Z" /></svg>
          </button>
          <button class="carousel-nav next" id="nextHotels">
            <svg viewBox="0 0 24 24"><path fill="currentColor" d="M8.59,7.41L13.17,12L8.59,16.58L10,18L16,12L10,6L8.59,7.41Z" /></svg>
          </button>
        </div>
      `;
      ...
      try {
        ...
        if (!hotelsCarouselEl || !hotelsWrapperEl) {
          hotelsSection.style.display = 'none';
          return;
        }

        const hotels = Array.isArray(venueData.hotels)
          ? venueData.hotels.filter(function (h) {
