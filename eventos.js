...
// Ambiente final: não usamos mais a pasta /site2026, então BASE_PATH fica vazio
const BASE_PATH = '';
...
function fixPath(path) {
  ...
  // Se já começar com "/", é caminho absoluto no domínio: mantém
  if (path.startsWith('/')) return path;

  // Se for caminho relativo ("eventos/..."), vira "/eventos/..."
  return '/' + path;
}
...
document.addEventListener('DOMContentLoaded', async function () {
  ...
  try {
    ...
    (function () {
      ...
      // Insere setas de navegação no carrossel de motivos (padrão igual ao carrossel de hotéis)
...
function renderError(message) {
  const target = document.getElementById('articleError') || document.getElementById('error');
  if (!target) {
  ...
  const text =
    message ||
    'Não foi possível carregar as informações deste evento. Tente novamente mais tarde ou fale com nossa equipe.';
  target.innerHTML = '<div class="error-box">' + text + '</div>';
}
